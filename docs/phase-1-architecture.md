...
- Every remaining mapped physical order line is selectable in that worksheet, regardless of whether its current source is Warehouse, Container, unassigned, Dropship, or Other. Source assignment continues to control fulfillment-record and inventory behavior; it never controls selection eligibility. When the canonical QuickBooks parent has an active same-invoice sibling with preserved physical lines, those remaining sibling lines appear on the canonical worksheet and retain their owning parent for fulfillment. A submission accepts lines from one parent only and creates its shipment, fulfillment, inventory, and activity evidence on that owner; it never copies, moves, or merges the sibling line into QuickBooks.
