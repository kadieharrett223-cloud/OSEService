...
- Orders search runs against the lightweight cached Orders projection across every lifecycle tab. The projection deduplicates active parents by `source_invoice_id` before lifecycle classification, preferring the QBO parent over OLD_ERP provenance, so stale siblings cannot appear as contradictory search results. A unique exact invoice search routes to its current lifecycle tab; partial/customer searches show each matching order with its lifecycle, and true invoice collisions with different source identities remain separate customer records. The 60-second projection cache is tagged and lifecycle-changing Server Actions invalidate the tag before redirecting.
...
- Orders currently preserves raw-parent visibility while sibling evidence has not been merged into a logical-order projection. `scripts/audit-orders-projection-canonicalization.mjs` and `scripts/snapshot-operational-data-readonly.mjs` are read-only release gates: no selected-parent or identity projection change may ship when the audit reports unexplained lost evidence or the protected-table fingerprints differ.
...
- ERP Health exposes `ACTIVE_DUPLICATE_PARENT_CONFLICT` for active QBO/OLD_ERP siblings with divergent product, quantity, or fulfillment evidence. Each finding shows the preserved parent IDs and comparison evidence with View Order and Investigate links; it intentionally has no automatic retirement control.
