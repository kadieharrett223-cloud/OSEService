...
- A saved `DROPSHIP` or `OTHER` source is displayed from the operational line itself, rather than inferred from warehouse allocations. Dropship/Other fulfillment records customer completion but never create `ON_FLOOR` or `SOLD` inventory transactions; only Warehouse/Floor fulfillment may move Olympic inventory.
- After a shipping session, run `node --env-file=.env.local scripts/audit-recent-fulfillment-integrity.mjs --since=<ISO timestamp>`. This read-only audit verifies each shipment line has matching fulfillment evidence, Warehouse/Floor lines recorded exact `ON_FLOOR -qty` and `SOLD +qty` events, Dropship/Other lines recorded no inventory events, and remaining customer demand matches the fulfillment ledger. It exits nonzero on any mismatch.
