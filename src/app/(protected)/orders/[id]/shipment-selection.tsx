...
export function ShipmentSelectionCheckbox({ line }: { line: SelectionLine }) {
  ...
  return <span className="mr-2 inline-flex items-center gap-1 align-middle" onClick={(event) => event.stopPropagation()} onKeyDown={(event) => event.stopPropagation()}><button type="button" aria-pressed={Boolean(selectedLine)} aria-label={`Include ${line.sku} in shipment`} onPointerDown={(event) => event.stopPropagation()} onClick={(event) => { event.stopPropagation(); toggle(line); }} className={`inline-flex h-4 w-4 items-center justify-center rounded-sm border text-[11px] font-bold leading-none ${selectedLine ? "border-[#2563eb] bg-[#2563eb] text-white" : "border-[#94a3b8] bg-white text-transparent"}`}>{selectedLine ? "✓" : ""}</button>{selectedLine ? <input type="number" min="1" max={line.remainingQty} value={selectedLine.quantity} onClick={(event) => event.stopPropagation()} onChange={(event) => setQuantity(line.id, Number(event.target.value))} className="w-16 rounded border border-[#cbd5e1] px-1.5 py-1 text-xs" aria-label={`Quantity of ${line.sku}`} /> : null}{line.inStock <= 0 ? <span className="ml-1 text-[11px] font-medium text-[#b45309]">Waiting / not in stock</span> : null}</span>;
}
