-- Forward-only accounting correction: fulfilled demand is derived from order lines, not SOLD.
-- Historical SOLD transactions remain untouched.
...
begin
  ...
  for v_line in select * from jsonb_array_elements(p_lines) loop
    ...
    select product_id, greatest(coalesce(approved_qty, 0), coalesce(ordered_qty, 0)), coalesce(fulfilled_qty, 0),
           coalesce(nullif(trim(upper(fulfillment_source)), ''), 'WAREHOUSE'), fulfillment_supplier, fulfillment_reference, fulfillment_tracking
      into v_product_id, v_demand, v_fulfilled, v_source, v_supplier, v_reference, v_line_tracking
    ...
    update public.shipping_order_lines
      set fulfilled_qty = v_next,
          ...
          allocation_status = case when v_source in ('DROPSHIP', 'OTHER') then 'UNALLOCATED' else allocation_status end
      where id = v_line_id;

    if v_source in ('DROPSHIP', 'OTHER') then delete from public.inventory_allocations where shipping_order_line_id = v_line_id; end if;
    insert into public.fulfillments(shipping_order_line_id, fulfilled_qty, fulfilled_at, shipment_number, carrier, tracking_number, reason, source_event_key, fulfillment_type)
      values (v_line_id, v_quantity, coalesce(p_shipped_at, now()), v_shipment_number,
        case when v_source = 'DROPSHIP' then coalesce(nullif(trim(v_supplier), ''), nullif(trim(p_carrier), '')) else nullif(trim(p_carrier), '') end,
        case when v_source = 'DROPSHIP' then coalesce(nullif(trim(v_line_tracking), ''), nullif(trim(p_tracking_number), '')) else nullif(trim(p_tracking_number), '') end,
        case when v_source = 'DROPSHIP' then 'Dropship fulfillment completed' when v_source = 'OTHER' then 'Other fulfillment completed' else 'Order shipment completed' end,
        'ORDER_SHIPMENT:' || v_shipment_id::text || ':' || v_line_id::text,
        case when v_source in ('DROPSHIP', 'OTHER') then v_source else 'SHIPMENT' end);

    if v_source not in ('DROPSHIP', 'OTHER') then
...
create or replace function public.edit_order_shipment(
  p_shipment_id uuid, p_order_id uuid, p_shipped_at timestamptz, p_carrier text, p_tracking_number text,
  p_notes text, p_lines jsonb, p_actor_id uuid default null
)
returns uuid
language plpgsql
security definer
...
begin
  ...
  for v_line in select * from jsonb_array_elements(p_lines) loop
    ...
    insert into shipment_edit_lines(shipping_order_line_id, requested_qty) values (v_line_id, v_requested_qty)
  ...
  for v_line_id in select l.id from public.shipping_order_lines l where l.shipping_order_id = p_order_id for update loop
    select coalesce(e.requested_qty, 0) into v_requested_qty from shipment_edit_lines e where e.shipping_order_line_id = v_line_id;
    v_requested_qty := coalesce(v_requested_qty, 0);
    select l.product_id, greatest(coalesce(l.approved_qty, 0), coalesce(l.ordered_qty, 0)), coalesce(l.fulfilled_qty, 0), coalesce(nullif(trim(upper(l.fulfillment_source)), ''), 'WAREHOUSE')
      into v_product_id, v_demand, v_current_fulfilled, v_source from public.shipping_order_lines l where l.id = v_line_id;
    ...
    if v_requested_qty > 0 then
      insert into public.order_shipment_lines(shipment_id, shipping_order_line_id, quantity) values (p_shipment_id, v_line_id, v_requested_qty)
    ...
    update public.shipping_order_lines
      set fulfilled_qty = coalesce(v_current_fulfilled, 0) + v_delta,
          fulfillment_status = case when coalesce(v_current_fulfilled, 0) + v_delta >= v_demand then 'FULFILLED' when coalesce(v_current_fulfilled, 0) + v_delta > 0 then 'PARTIALLY_FULFILLED' else 'PENDING' end,
          warehouse_status = case when coalesce(v_current_fulfilled, 0) + v_delta >= v_demand then 'FULFILLED' when coalesce(v_current_fulfilled, 0) + v_delta > 0 then 'PARTIALLY_FULFILLED' else 'APPROVED' end
      where id = v_line_id;
    v_event_key := 'ORDER_SHIPMENT_EDIT:' || p_shipment_id::text || ':' || v_line_id::text || ':' || gen_random_uuid()::text;
    insert into public.fulfillments(shipping_order_line_id, fulfilled_qty, fulfilled_at, shipment_number, carrier, tracking_number, reason, source_event_key, fulfillment_type)
      values (v_line_id, v_delta, coalesce(p_shipped_at, v_shipment.shipped_at), v_shipment.shipment_number, nullif(trim(p_carrier), ''), nullif(trim(p_tracking_number), ''), 'Shipment contents edited', v_event_key, case when v_source in ('DROPSHIP', 'OTHER') then v_source else 'SHIPMENT' end);
    if v_source not in ('DROPSHIP', 'OTHER') then
      select coalesce(sum(delta), 0) into v_current_floor from public.inventory_transactions where product_id = v_product_id and bucket = 'ON_FLOOR';
      insert into public.inventory_transactions(product_id, bucket, delta, before_qty, after_qty, reason, source_type, source_event_key, shipping_order_line_id, actor_id)
        values (v_product_id, 'ON_FLOOR', -v_delta, v_current_floor, v_current_floor - v_delta, 'Shipment contents edited', 'FULFILLMENT', v_event_key || ':ON_FLOOR', v_line_id, p_actor_id);
    end if;
  end loop;
