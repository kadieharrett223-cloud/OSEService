...
async function ProtectedSidebar() {
...
function SidebarFallback() {
  return <aside className="border-r border-[#232833] bg-[#10141b]" />;
}
    ...
    <header className="border-b border-[#e3e6ea] bg-white px-4 py-3 shadow-sm md:px-6">
      <div className="flex items-center gap-3">
        ...
        <div className="hidden items-center gap-2 rounded-lg border border-[#e5e7eb] bg-white px-2.5 py-1.5 text-xs md:flex">
          ...
          {!isQboConnected ? <Link href="/settings" className="font-semibold text-[#d50917] hover:underline">Connect</Link> : null}
        </div>
        <details className="relative">
          <summary className="list-none cursor-pointer [&::-webkit-details-marker]:hidden"><span className="inline-flex h-9 w-9 cursor-pointer items-center justify-center rounded-full bg-[#d50917] text-sm font-semibold text-white">{userName.slice(0, 1).toUpperCase()}</span></summary>
          <div className="absolute right-0 z-20 mt-2 w-44 rounded-xl border border-[#e5e7eb] bg-white p-2 shadow-lg">
            <p className="px-2 py-1 text-xs text-[#6b7280]">Signed in as</p><p className="px-2 pb-2 text-sm font-semibold text-[#1f2937]">{userName}</p>
            <form action={signOutAction}><button type="submit" className="btn-danger w-full text-left">Sign Out</button></form>
          </div>
        </details>
      </div>
    </header>
