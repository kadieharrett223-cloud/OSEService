...
- Orders search runs against the lightweight cached Orders projection across every lifecycle tab. A unique exact invoice search routes to its current lifecycle tab; partial/customer searches show each matching order with its lifecycle, and invoice collisions remain separate customer records. The 60-second projection cache is tagged and lifecycle-changing Server Actions invalidate the tag before redirecting.
