  ...
  return (
    <div className="space-y-5">
      ...
      <section className="overflow-x-auto rounded-xl border border-[#e5e7eb] bg-white">
        ...
        <table className="w-full min-w-[1080px] table-fixed text-left text-sm"><colgroup><col className="w-[8%]" /><col className="w-[17%]" /><col className="w-[17%]" /><col className="w-[9%]" /><col className="w-[15%]" /><col className="w-[10%]" /><col className="w-[9%]" /><col className="w-[15%]" /></colgroup><thead className="bg-[#f8fafc]"><tr className="border-b border-[#e5e7eb] text-xs font-semibold uppercase tracking-[0.06em] text-[#64748b]"><th className="px-3 py-3">Invoice</th><th className="px-3 py-3">Customer</th><th className="px-3 py-3">QBO SKU / Qty</th><th className="px-3 py-3">Paid status</th><th className="px-3 py-3">ERP representation</th><th className="px-3 py-3">Mapping</th><th className="px-3 py-3">Terminal</th><th className="px-3 py-3">Action</th></tr></thead><tbody>{historicalIntakeRows.map((row) => <tr key={row.qboInvoiceLineId} className="border-b border-[#f1f5f9] last:border-0 align-top"><td className="px-3 py-3 font-semibold">{row.invoice ?? "-"}</td><td className="truncate px-3 py-3" title={row.customer ?? "Customer pending"}>{row.customer ?? "Customer pending"}</td><td className="truncate px-3 py-3" title={`${row.sku ?? "Unidentified"} x ${row.quantity}`}>{row.sku ?? "Unidentified"} x {row.quantity}</td><td className="px-3 py-3">{row.paymentStatus ?? "Unknown"}</td><td className="px-3 py-3">{row.representation}</td><td className="px-3 py-3">{row.mappingStatus}</td><td className="px-3 py-3">{row.terminalStatus}</td><td className="px-3 py-3">{row.action === "MAP_PRODUCT" ? <Link className="btn-secondary text-xs" href={`/product-mappings?source_sku=${encodeURIComponent(row.sku ?? "")}`}>Mapping Review</Link> : <HistoricalIntakeAction qboInvoiceLineId={row.qboInvoiceLineId} />}</td></tr>)}{historicalIntakeRows.length === 0 ? <tr><td colSpan={8} className="px-4 py-6 text-sm text-[#64748b]">No paid historical QBO intake gaps are currently eligible for review.</td></tr> : null}</tbody></table>
      </section>
