...
export default async function ContainersPage({
  ...
  const itemsNeedingMapping = (containers ?? []).flatMap((container) => {
    ...
    return (typedContainer.container_lines ?? [])
      .filter((line) => {
        const status = String(line.product_mapping_status ?? "MAPPED").toUpperCase();
        return status !== "MAPPED" || !line.product_id || !line.products?.sku;
      })
      .map((line) => ({
        containerNumber: typedContainer.container_number,
        lineId: line.id,
        sourceLineRef: line.source_line_ref ?? "—",
        mappingStatus: String(line.product_mapping_status ?? "UNMAPPED").toUpperCase(),
        sku: line.products?.sku ?? "Unknown SKU",
        qty: Number(line.ordered_qty ?? 0),
      }));
  }).sort((a, b) => {
    const containerCompare = String(a.containerNumber).localeCompare(String(b.containerNumber), undefined, { numeric: true });
