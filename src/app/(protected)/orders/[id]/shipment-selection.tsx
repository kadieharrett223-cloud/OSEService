...
type ContextValue = {
  ...
  start: () => void;
  cancel: () => void;
  toggle: (line: SelectionLine) => void;
  setQuantity: (id: string, quantity: number) => void;
};
const SelectionContext = createContext<ContextValue | null>(null);

export function ShipmentSelectionProvider({
  children,
}: {
      ...
      current[id]
        ? {
            ...
            [id]: {
              ...
              quantity: Math.max(
                1,
                Math.min(current[id].remainingQty, quantity || 1),
              ),
            },
          }
        : current,
  ...
  return (
    <SelectionContext.Provider
      value={{ active, selected, start, cancel, toggle, setQuantity }}
    >
      {children}
    </SelectionContext.Provider>
  );
...
function useSelection() {
  ...
  if (!value)
    throw new Error(
      "Shipment selection controls must be inside ShipmentSelectionProvider",
    );
  return value;
}

export function ShipmentSelectionButton({
  pickupMode = false,
}: {
  pickupMode?: boolean;
}) {
  ...
  return active ? (
    <span className="rounded-full bg-[#fff7e6] px-3 py-1.5 text-xs font-semibold text-[#92400e]">
      Fulfilling
    </span>
  ) : (
    <button
      ...
      onClick={start}
      aria-label={pickupMode ? "Fulfill customer pickup" : "Fulfill customer shipment"}
    >
      Fulfill
    </button>
  );
    ...
    >
      ...
      {selectedLine ? (
        <input
          ...
          className="w-16 rounded border border-[#cbd5e1] px-1.5 py-1 text-xs"
...
export function ShipmentSelectionComposer({
  orderId,
  pickupMode = false,
}: {
  orderId: string;
  ...
  return (
    <div className="mt-4 rounded-xl border border-[#bfdbfe] bg-[#f8fbff] p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h3 className="text-lg font-semibold text-[#111827]">Fulfilling Order</h3>
          <p className="mt-1 text-sm text-[#475569]">
            Select the items and quantities being fulfilled.
          </p>
        </div>
      ...
      {selected.length === 0 ? (
        <p className="mt-3 text-sm text-[#64748b]">
          Select at least one remaining line above.
        </p>
      ) : (
        <form
          action={
        ...
        >
          ...
          {!pickupMode ? (
            <input
              ...
              value={`${orderId}:${selected.map((line) => `${line.id}-${line.quantity}`).join(",")}`}
            />
          ) : null}
          <p className="text-sm font-semibold text-[#334155]">
            {pickupMode ? "Customer picked up" : "Fulfillment recorded"}
          </p>
          ...
          {pickupMode ? (
            <>
              ...
              <label className="text-xs font-semibold text-[#64748b]">
                Pickup notes
                <input name="pickup_notes" className="input mt-1" />
              </label>
              <label className="text-xs font-semibold text-[#64748b]">
                Acknowledgment document
                <input
                  name="acknowledgment_document_id"
                  className="input mt-1"
              ...
              <label className="text-xs font-semibold text-[#64748b]">
                Restricted ID document
                <input
                  name="drivers_license_document_id"
                  className="input mt-1"
                  placeholder="Document ID from Documents"
                  required
                />
          ...
          <button type="submit" className="btn-primary">
            {pickupMode ? "Customer picked up" : "Fulfill items"}
          </button>
        </form>
      )}
    </div>
