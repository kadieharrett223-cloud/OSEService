...
- ERP Health renders the same live historical paid-QBO intake review population as a dedicated read-only table. It exposes immutable QBO invoice-line identity, payment, ERP representation, mapping, terminal state, and the shared classifier decision. Mapped rows remain review-only until a separately approved exact-line historical activation policy is implemented; unmapped rows link to Product Mapping Review and cannot create demand.
