...
begin
  ...
  for v_line in select * from jsonb_array_elements(p_lines) loop
    ...
    select product_id,
           greatest(coalesce(approved_qty, 0), coalesce(ordered_qty, 0)),
           coalesce(fulfilled_qty, 0),
           coalesce(nullif(trim(upper(fulfillment_source)), ''), 'WAREHOUSE'),
    ...
    if v_source in ('DROPSHIP', 'OTHER') then
      delete from public.inventory_allocations where shipping_order_line_id = v_line_id;
    end if;
    ...
    insert into public.fulfillments(shipping_order_line_id, fulfilled_qty, fulfilled_at, shipment_number, carrier, tracking_number, reason, source_event_key, fulfillment_type)
      values (
        v_line_id, v_quantity, coalesce(p_shipped_at, now()), v_shipment_number,
